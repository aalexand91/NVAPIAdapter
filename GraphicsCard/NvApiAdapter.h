...
namespace GraphicsCards
{
	...
	namespace NvApiAdapter
	{
		...
		/// </returns>
		GRAPHICS_CARD_API NvAPI_Status Initialize();
		...
		/// </returns>
		GRAPHICS_CARD_API NvAPI_Status Unload();
		...
		/// </returns>
		GRAPHICS_CARD_API NvAPI_Status GetFullName(const NvPhysicalGpuHandle physicalHandler, char* name);
		...
		GRAPHICS_CARD_API NvAPI_Status GetGpuType(const NvPhysicalGpuHandle physicalHandler, NV_GPU_TYPE* gpuType);
	}
}